...
def main():
    ...
    ap.add_argument("--max-shape-thdn-db", type=float, default=None,
                    help="exit non-zero if a shipped-fix 3/4-note chord "
                         "(section 5c) exceeds this THD+N; regression gate")
    args = ap.parse_args()
    ...
    R("")
    R("**The fix (`WavetableSynth.WT_HEADROOM = 0.5`) is applied.** Section 5c "
      "measures it through the shipped `_make_notes` path; the rest of this "
      "doc is the diagnosis it came from.")
    R("")
    ...
    # --- 5c: the shipped fix through the real _make_notes path -----------
    ship_rows = []
    for n in (3, 4, 5):
        rawc, fixc = "ship_raw_n%d" % n, "ship_fixed_n%d" % n
        ncid = "notional_n%d" % n
        if rawc not in metrics or fixc not in metrics or ncid not in manifest:
            continue
        ref = steady(get(ncid)[0])
        ref_inv = 1.0 / manifest[ncid]["wt_scale"]
        for cid in (rawc, fixc):
            c = manifest[cid]
            s = steady(get(cid)[0])
            ideal = ref * (ref_inv * c["wt_headroom"])
            L = min(len(s), len(ideal))
            _, shp, _ = thdn(s[:L], ideal[:L])
            if cid == fixc and n <= 4 and args.max_shape_thdn_db is not None \
                    and shp > args.max_shape_thdn_db:
                fail = True
            lvl = c["mixer_level"]
            ship_rows.append([
                "WT_HEADROOM %.2f, level %.2f" % (c["wt_headroom"], lvl),
                n, int(float(np.max(np.abs(s))) * lvl),
                int(np.sqrt(np.mean((s * lvl) ** 2))),
                "%.1f" % shp, "%.2f%%" % (100 * 10 ** (shp / 20))])
    if ship_rows:
        R("## 5c. The shipped fix (real `_make_notes`, no amplitude override)")
        R("")
        R("`WavetableSynth.WT_HEADROOM` 1.0 vs 0.5 with `mixer.voice[0].level` "
          "0.25 -> 0.5, pressed with `note_on(n, 127)` so the class default is "
          "what runs. Peak/RMS are post the mixer level (scalar; 5b shows it "
          "matches a real Mixer).")
        R("")
        R(fmt_table(["config", "N", "peak", "RMS", "THD+N dB", "THD+N %"],
                    ship_rows))
        R("")
    ...
    # --- recommendation -----------------------------------------------
    R("## 7. What was done, and what to watch")
    R("")
    R("- **Applied:** `WavetableSynth.WT_HEADROOM = 0.5` (in `_make_notes`'s "
      "`amplitude`). Section 5c: a 3-4 note chord goes from ~10-13% THD+N to "
      "<0.1%, a worst-case 5-note from ~17% to ~0.9%, RMS within +1 dB. "
      "`examples/synthtools_wavetable_chords.py` pairs it with "
      "`mixer.voice[0].level` 0.5 (up from 0.25) as makeup and A/Bs the two on "
      "knobB. Someone doing dense 6+ note pads sets `WT_HEADROOM = 0.35` and "
      "`level` ~0.7.")
    R("- **Scope of the numbers:** one wavetable (BRAIDS02), its full-scale "
      "frame (4), phase-aligned press: worst case on every axis. A mid-table "
      "frame or a de-tuned voicing distorts less.")
    if Hneed:
        R("- Worst-case headroom (`H_needed` above): ~%.2f for 3 notes, ~%.2f "
          "for 4, ~%.2f for 5. `WT_HEADROOM` 0.5 clears 3-4; a 5+ note chord on "
          "the worst frame still just tips the knee (hence the 0.9%%)."
          % (Hneed.get(3, 0.6), Hneed.get(4, 0.5), Hneed.get(5, 0.45)))
    R("- **rp2040 caveat:** the knee (28000) is identical on every port, but "
      "the slope past it is steeper there (`MAX_CHANNELS` 24 vs this build's "
      "14), so the residual on 5+ note chords is worse on hardware than 5c "
      "shows. Confirm on a board.")
    R("- `mixer.voice[0].level` alone (\"control\" row) does not help: the "
      "limiter already acted upstream in synthio's int16 output. 5b confirms a "
      "real `audiomixer.Mixer` matches the scalar model, so the 0.5 makeup is "
      "safe.")
    R("- `/sqrt(N)` would be cleaner still but needs every sounding note's "
      "`amplitude` rewritten at each note-on (O(polyphony), against the "
    ...
    if fail:
        print("FAIL: shipped-fix chord (section 5c) exceeds --max-shape-thdn-db")
        sys.exit(1)
